using System.Collections.Generic;

namespace Windows
{
    /// <summary>
    /// The design-time data for a <see cref="ServerListViewModel"/>
    /// </summary>
    public class ServerListDesignModel : ServerListViewModel
    {
        #region Singleton

        /// <summary>
        /// A single instance of the design model
        /// </summary>
        public static ServerListDesignModel Instance => new ServerListDesignModel();

        #endregion

        #region Constructor

        /// <summary>
        /// Default constructor
        /// </summary>
        public ServerListDesignModel()
        {
            Items = new List<ServerListItemViewModel>
            {
                new ServerListItemViewModel
                {
                    Name = "Luke",
                    Initials = "LM",
                    Message = "This chat app is awesome! I bet it will be fast too",
                    ProfilePictureRGB = "3099c5",
                    NewContentAvailable = true
                },
                new ServerListItemViewModel
                {
                    Name = "Jesse",
                    Initials = "JA",
                    Message = "Hey dude, here are the new icons",
                    ProfilePictureRGB = "fe4503"
                },
                new ServerListItemViewModel
                {
                    Name = "Parnell",
                    Initials = "PL",
                    Message = "The new server is up, got 192.168.1.1",
                    ProfilePictureRGB = "00d405",
                    IsSelected = true
                },
                new ServerListItemViewModel
                {
                    Name = "Luke",
                    Initials = "LM",
                    Message = "This chat app is awesome! I bet it will be fast too",
                    ProfilePictureRGB = "3099c5"
                },
                new ServerListItemViewModel
                {
                    Name = "Jesse",
                    Initials = "JA",
                    Message = "Hey dude, here are the new icons",
                    ProfilePictureRGB = "fe4503"
                },
                new ServerListItemViewModel
                {
                    Name = "Parnell",
                    Initials = "PL",
                    Message = "The new server is up, got 192.168.1.1",
                    ProfilePictureRGB = "00d405"
                },
                //                new ServerListItemViewModel
                //{
                //    Name = "Luke",
                //    Initials = "LM",
                //    Message = "This chat app is awesome! I bet it will be fast too",
                //    ProfilePictureRGB = "3099c5"
                //},
                //new ServerListItemViewModel
                //{
                //    Name = "Jesse",
                //    Initials = "JA",
                //    Message = "Hey dude, here are the new icons",
                //    ProfilePictureRGB = "fe4503"
                //},
                //new ServerListItemViewModel
                //{
                //    Name = "Parnell",
                //    Initials = "PL",
                //    Message = "The new server is up, got 192.168.1.1",
                //    ProfilePictureRGB = "00d405"
                //},
            };
        }

        #endregion
    }
}
